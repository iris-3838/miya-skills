...
git -C /workspace/llm-kb.miya-lis.net add -A
git -C /workspace/llm-kb.miya-lis.net commit -m "feat: <summary>"
git -C /workspace/llm-kb.miya-lis.net push origin main
```
...
### /workspace/llm-kb.miya-lis.net 書き込み不可（解決済み）

**過去の制約**: Docker hermes ユーザー (uid 10000) が KB ディレクトリに直接書き込めなかった。
- `/workspace/` 自体は 777、しかし `llm-kb.miya-lis.net/` 以下は `775` (owner=1000のみ書き込み可)
- **解決**: `sudo chmod -R o+w /workspace/llm-kb.miya-lis.net` を実行済み
...
- **回避策 A:** `GIT_CONFIG_NOSYSTEM=1 HOME=/tmp` を git コマンドの前に付与する
  ...
  GIT_CONFIG_NOSYSTEM=1 HOME=/tmp git config --global --add safe.directory /workspace/llm-kb.miya-lis.net

  # 以降はこのプレフィックスで git 操作
  GIT_CONFIG_NOSYSTEM=1 HOME=/tmp git -C /workspace/llm-kb.miya-lis.net status
  GIT_CONFIG_NOSYSTEM=1 HOME=/tmp git -C /workspace/llm-kb.miya-lis.net log --oneline -5
  ```
- **回避策 B:** `GIT_CONFIG_GLOBAL=/workspace/.gitconfig` — 書き込み可能なカスタムconfigで上書き
  ```bash
  cat > /workspace/.gitconfig << 'EOF'
  [safe]
      directory = /workspace/llm-kb.miya-lis.net
  EOF
  export GIT_CONFIG_GLOBAL=/workspace/.gitconfig
  git config user.email "seimiya@example.com"
  git config user.name "seimiya"
  cd /workspace/llm-kb.miya-lis.net && git pull origin main
  ```
