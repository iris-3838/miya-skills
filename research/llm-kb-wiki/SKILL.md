...
Knowledge base built on Karpathy's [llm-wiki](https://github.com/karpathy/llm-wiki) concept. Hosted at `llm-kb.miya-lis.net`.
...
### Wiki Location
```
/opt/data/workspace/llm-kb.miya-lis.net/
```
...
```bash
sudo chmod -R o+w /opt/data/workspace/llm-kb.example.com
```
...
**Sync with GitHub:**
```bash
cd /opt/data/workspace/llm-kb.example.com
...
2. `/opt/data/workspace/` などユーザーがアクセス可能な場所に保存
3. その旨をユーザーに伝え、vision-capable なモデルで画像を読み取ってもらうよう依頼

```bash
OUTDIR="/opt/data/workspace/$(basename $PDF .pdf)_pages"
mkdir -p "$OUTDIR"
python3 -c "
import fitz, sys, os
...
| ディレクトリ | 内容 | 作成条件 |
|-------------|------|---------|
| `concepts/` | 概念・分析結果・ワークフロー文書・**研究計画** | 独立した知識単位 |
...
| `raw/surveys/` | 定期的な調査レポート（backnumber/ 等） | ウェブスクレイピング・APIによる定期収集結果 |
| `raw/corpora/` | RAG corpus entry symlinks | 外部辞書・教科書エントリへの参照（下記参照） |

### raw/corpora — RAG corpus連携

`raw/corpora/` 以下に、RAGシステム（ziten / rag CLI）のデータセットを symlink で取り込む:

```
raw/corpora/
  ├── lisdict/  →  ../../../lisdict/entries/   (1,796件, 図書館情報学辞典)
  ├── ieko/     →  ../../../ieko/entries/       (127件, ISKO百科事典)
  └── d2l/      →  ../../../d2l/entries/        (470件, 深層学習教科書)
```

用途:
- KB内の分析ページから `^[raw/corpora/lisdict/FRBR.md]` で出典引用
- agentがraw/corpora/のエントリを読み込んでconcepts/に合成
- gitはsymlink自体を追跡（mode 120000）
...
```bash
# GIT_CONFIG_GLOBAL を使う場合
export GIT_CONFIG_GLOBAL=/opt/data/workspace/.gitconfig
git -C /opt/data/workspace/llm-kb.miya-lis.net add -A
git -C /opt/data/workspace/llm-kb.miya-lis.net commit -m "feat: <summary>"
git -C /opt/data/workspace/llm-kb.miya-lis.net push origin main
```
...
### /opt/data/workspace/llm-kb.miya-lis.net 書き込み不可（解決済み）

**過去の制約**: Docker hermes ユーザー (uid 10000) が KB ディレクトリに直接書き込めなかった。
- `/opt/data/workspace/` 自体は 777、しかし `llm-kb.miya-lis.net/` 以下は `775` (owner=1000のみ書き込み可)
- **解決**: `sudo chmod -R o+w /opt/data/workspace/llm-kb.miya-lis.net` を実行済み
- 現在は agent が Agent Zone 全ファイルに直接書き込み可能

### Git Permission Denied (`~/.gitconfig`)
`~/.gitconfig` へのアクセス権限がないため、git 操作が失敗する（exit 128）。
- **回避策 A:** `GIT_CONFIG_NOSYSTEM=1 HOME=/tmp` を git コマンドの前に付与する
  ```bash
  # 初回だけ safe.directory 登録が必要
  GIT_CONFIG_NOSYSTEM=1 HOME=/tmp git config --global --add safe.directory /opt/data/workspace/llm-kb.miya-lis.net

  # 以降はこのプレフィックスで git 操作
  GIT_CONFIG_NOSYSTEM=1 HOME=/tmp git -C /opt/data/workspace/llm-kb.miya-lis.net status
  GIT_CONFIG_NOSYSTEM=1 HOME=/tmp git -C /opt/data/workspace/llm-kb.miya-lis.net log --oneline -5
  ```
- **回避策 B:** `GIT_CONFIG_GLOBAL=/opt/data/workspace/.gitconfig` — 書き込み可能なカスタムconfigで上書き
  ```bash
  cat > /opt/data/workspace/.gitconfig << 'EOF'
  [safe]
      directory = /opt/data/workspace/llm-kb.miya-lis.net
  EOF
  export GIT_CONFIG_GLOBAL=/opt/data/workspace/.gitconfig
  git config user.email "seimiya@example.com"
  git config user.name "seimiya"
  cd /opt/data/workspace/llm-kb.miya-lis.net && git pull origin main
  ```
