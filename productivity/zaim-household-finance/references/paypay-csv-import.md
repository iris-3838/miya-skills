...
Location: `/workspace/filter_paypay_csv.py` (host: `~/workspace/filter_paypay_csv.py`)

### Usage
```bash
# Dry-run (preview what would be removed)
python3 ~/workspace/filter_paypay_csv.py PayPay.csv --dry-run

# Filter and save
python3 ~/workspace/filter_paypay_csv.py PayPay.csv -o PayPay_zaim.csv

# Custom column name (if different locale)
python3 ~/workspace/filter_paypay_csv.py PayPay.csv -o output.csv --method-col "取引方法"
```
